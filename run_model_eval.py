...
import argparse, inspect, json, shutil, sys, time
...
def eval_models(path):
    ...
    The model the agent RUNS ON (CLAUDE_MODEL) and the models it is EVALUATING
    are different things — conflating them would let the report name one model
    in the cost analysis and a different one in the summary.
    """
    import csv as _csv
    try:
...
SYSTEM_PROMPT = (
    "You are AirClaude. Call tools one at a time. Never write text between tool calls. "
...
def _deterministic_fallback(missing):
    ...
    itself — the tools are self-hydrating, so it only needs file_path and the
    two model names. If the model stalls anyway, finish the run rather than
    dying mid-demo.
...
def run_agent(client):
    banner("AirClaude Eval Agent — Model Migration Analysis Starting")
    model_a, model_b = eval_models(str(UPSTREAM.resolve()))
    log_t(f"Comparing     : {model_a}  ->  {model_b}")
    log_t(f"Agent runs on : {CLAUDE_MODEL}  ({get_provider()})")
    ...
    for iteration in range(1, MAX_ITER + 1):
        ...
        if response is None:
            log_r("Claude call failed — retrying in 2s…")
            time.sleep(2)
            continue
        ...
        for tc in tool_uses:
            ...
            if name not in TOOL_REGISTRY:
                ...
                tool_result_blocks.append(tool_result_block(tc["id"], result_content, is_error=True))
                div()
                continue
            ...
            tool_result_blocks.append(
                tool_result_block(tc["id"], trim_for_history(name, result_content))
            )
            div()
    ...
    banner("AirClaude Eval Agent — Analysis Complete")
...
def main():
    parser = argparse.ArgumentParser(description="AirClaude model eval demo runner")
