...
Three tasks:
  ...
  [1] ingest_eval_results → [2] airclaude_eval_agent → [3] publish_report

Task 1: Loads the eval CSV, packages context.
Task 2: AirClaudeOperator — compares models, detects regressions,
...
with DAG(
    dag_id="airclaude_model_eval_demo",
    description="AirClaude — Model Migration Eval (GPT-4o vs Nemotron candidate)",
    default_args=default_args,
    start_date=datetime(2024, 1, 1),
    schedule=None,
    catchup=False,
    tags=["airclaude", "model-eval", "migration", "demo"],
) as dag:
    ...
    def ingest_eval_results(**kwargs):
        ...
        print(f"[ingest] Handing off to AirClaude eval agent.")
    ...
    def publish_report(**kwargs):
        ti     = kwargs["ti"]
        result = ti.xcom_pull(task_ids="airclaude_eval_agent")
        ...
        print(f"{BOLD}{TEAL}  AirClaude — Model Migration Report Ready{RESET}")
