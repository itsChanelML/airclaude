"""
Tool Registry — Model Migration Eval Agent
---------------------------------------------------
...
def draft_migration_report(input: DraftMigrationReportInput) -> AgentResult:
    ...
    lines.append(f"Generated by AirClaude Eval Agent")
    lines.append("")
    ...
    lines.append("")
    lines.append("—")
    lines.append("AirClaude Eval Agent | Automated Model Migration Analysis")
...
def generate_summary(input: GenerateSummaryInput) -> AgentResult:
    lines = [
        f"AirClaude eval run complete — {datetime.now().strftime('%Y-%m-%d %H:%M')}."
    ]
...
# ── History trimming ───────────────────────────────────────────────────────────
# Same purpose as in triage_tools: keep the message history small enough that
# the model keeps calling tools instead of stalling. Shared by run_model_eval.py
# and the Airflow AirClaudeOperator so both behave identically.
