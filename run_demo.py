...
Environment:
  ...
  # Or point at Bedrock / Vertex instead of the direct API:
  export CLAUDE_PROVIDER=bedrock   # or vertex
"""
...
from claude_tool_adapter import to_claude_tools, extract_tool_uses, extract_text, tool_result_block

sys.path.insert(0, str(ROOT))
from claude_env import get_model, get_client_and_error, get_provider
...
CLAUDE_TOOLS = to_claude_tools(TOOL_SCHEMAS)
MAX_ITER     = 14
MAX_TOKENS   = 1024
...
SYSTEM_PROMPT = (
    "You are AirClaude, an autonomous operations agent for NYC 311.\n\n"
    "How you work:\n"
    ...
    for attempt, timeout in enumerate(timeouts, 1):
        try:
            ...
            if attempt < len(timeouts):
                log_a(f"Claude call failed (attempt {attempt}/{len(timeouts)}): {e} — retrying…")
                time.sleep(3)
            else:
                log_r(f"Claude API error: {e}")
                return None
...
def run_agent(goal: str, context: dict, client, custom_goal: bool = False):
    banner("AirClaude Agent — Morning Triage Starting")
    log_t(f"Goal       : {goal[:110]}…")
    log_t(f"Model      : {CLAUDE_MODEL}  ({get_provider()})")
    log_t(f"As of      : {datetime.now().strftime('%B %d, %Y at %I:%M %p')}")
    div()

    # Claude's system prompt is a top-level request param, not a message — the
    ...
    for iteration in range(1, MAX_ITER + 1):
        log_p(f"[Iteration {iteration}] Calling AirClaude…")

        response = call_claude(client, messages)
        if response is None:
            log_a("Claude call failed — retrying in 2s…")
            time.sleep(2)
            continue
        ...
        for tc in tool_uses:
            ...
            if name not in TOOL_REGISTRY:
                ...
                tool_result_blocks.append(tool_result_block(tc["id"], history_content, is_error=True))
                div()
                continue
            ...
            tool_result_blocks.append(tool_result_block(tc["id"], history_content))
            div()

        # All tool results for this turn go back as one user message —
        # Claude expects every tool_use in a turn matched by a tool_result
    ...
    # ── Fallback ──────────────────────────────────────────────────────────────
    # The briefings are the demo, and generate_summary is the final structured
    # output. If the agent skipped either, finish the run deterministically
    # rather than dying on stage.
    ...
    if custom_goal and answer and "draft_supervisor_briefing" not in called:
        banner("AirClaude Agent — Question Answered")
    ...
    if final_result:
        banner("AirClaude Agent — Triage Complete")
        if not missing:
...
def main():
    parser = argparse.ArgumentParser(description="AirClaude NYC 311 productivity agent demo")
    ...
    client, client_error = get_client_and_error()
    if client_error:
        print(f"{RED}Error: {client_error}{RESET}")
        sys.exit(1)
