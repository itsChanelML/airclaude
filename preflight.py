...
Exit code 0 means the demo is ready to run. Non-zero means read the output.
"""
...
def check_dependencies():
    ...
    for module, why in (("anthropic", "Claude API calls"),
...
def check_client():
    ...
    from claude_env import get_provider, get_client_and_error
    provider = get_provider()
    ok("CLAUDE_PROVIDER", provider)

    client, error = get_client_and_error()
    if error:
        fail(f"{provider} client", error.splitlines()[0])
        return None
    ok(f"{provider} client", "built successfully")
    return client
...
def check_model(client):
    section("Model")
    if client is None:
        fail("model reachability", "skipped — no usable client")
        return

    from claude_env import get_model
    model = get_model()
...
def check_adapter():
    ...
    if names == expected and all("input_schema" in t for t in claude_tools):
        ...
        fail("openai -> claude conversion", f"got {names}, expected {expected}")
...
def main():
    parser = argparse.ArgumentParser(description="AirClaude pre-flight check")
    parser.add_argument("--no-airflow", action="store_true",
                        help="skip the Airflow DAG parse check")
    args = parser.parse_args()

    print(f"\n{BOLD}{TEAL}AirClaude — pre-flight check{RESET}")
    ...
    if failures:
        ...
        print(f"\n{GRAY}Fix these before demoing.{RESET}\n")
        return 1
    ...
    print(f"{GREEN}{BOLD}READY — all checks passed.{RESET}\n")
    return 0
